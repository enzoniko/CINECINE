...
from app.Backend.helpers import get_most_empty_room, load_objects, most_empty, store_objects, lista_strings_para_string
...
filmes: List[Filme] = list(load_objects('app/storage/filmes.pkl')) or store_objects([
        Filme("Raya and the last Dragon"), 
        Filme("Homens de Preto")
    ], "app/storage/filmes.pkl")

sessoes: List[Sessao] = list(load_objects('app/storage/sessoes.pkl')) or store_objects([
    Sessao(nome = 'Raya and the last Dragon', classificacao='livre', legenda = False, DDD = False, horarios = ['15:00', '20:00']), 
    Sessao(nome = 'Raya and the last Dragon', classificacao='livre', legenda = True, DDD = False, horarios = ['15:00', '23:00']), 
    Sessao(nome = 'Homens de Preto', classificacao='+16', legenda = True, DDD = False, horarios = ['22:00', '23:00'])
], "app/storage/sessoes.pkl")
...
def poltronas(id_sessao, horario):
    ...
    quantidade_lugares_disponiveis_sala_mais_vazia: int = most_empty(
        [
            [
                sala.cronograma[f"{id_sessao} {horario}"] 
    ...
    sala: Sala = get_most_empty_room(session, salas, quantidade_lugares_disponiveis_sala_mais_vazia)
    poltronas: List[List[int]] = [
        sala.cronograma[f"{id_sessao} {horario}"][i-1][::-1] 
        for i in range(len(sala.cronograma[f"{id_sessao} {horario}"]), 0, -1)
    ]
    ...
    if form.validate_on_submit():
        if form.meias.data > len(ingressos):
            ...
            poltronas_a_preencher: List[str] = [
                letras[::-1][:len(poltronas)][int(ingressos[i].split()[0])]+[
                    str(x+1) 
                    for x in range(len(poltronas[0]))
                ][::-1][int(ingressos[i].split()[1])] 
                for i in range(len(ingressos))
            ]
            sala.preencher_poltronas(poltronas_a_preencher, id_sessao, horario)
            pagamento: Pagamento = Pagamento(len(ingressos), form.meias.data, forma = form.maneira.data, extra = 1 if session.DDD else 0)
...
def pagamento(pagamento_id, id_sessao, horario, poltronas):
    ...
    return render_template(
        ...
        sala = [
            salas.index(sala) 
            for sala in salas 
            if int(id_sessao) in [sessao.id for sessao in sala.sessoes]
        ][0]
    )
...
def adminMenu():
    return render_template(
        ...
        title = "Admin Menu", 
        dados_faturamento = {
            "Valor faturado": f'R$ {sum(pagamento.valor for pagamento in pagamentos)},00' if pagamentos else 'R$ 0,00',
            "Total de Ingressos": sum(pagamento.ingressos for pagamento in pagamentos) if pagamentos else 0,
            "Total de Ingressos Inteiros": sum(pagamento.ingressos - pagamento.meias for pagamento in pagamentos) if pagamentos else 0, 
            "Total de Meias-entradas": sum(pagamento.meias for pagamento in pagamentos) if pagamentos else 0
        },
        salas = salas,  
        filmes = [(filmes.index(filme) + 1, filme) for filme in filmes], 
        sessions = create_sessions_dict(), 
...
def editar_sessao(id_sessao):
    ...
    form: EditarSessaoForm = EditarSessaoForm(
        ...
        horarios = {
            datetime.strptime(horario, '%H:%M'): datetime.strptime(horario, '%H:%M') 
